...
{
    int one_year = 60*24*365;
    CAmount nSubsidy = 7 * COIN;

    if (nPrevHeight == 1) {
        nSubsidy=316000000 * COIN;
    }else if(nPrevHeight < one_year * 1){
        nSubsidy=100 * COIN;
    }else if(nPrevHeight < one_year * 2){
        nSubsidy=80 * COIN;
    }else if(nPrevHeight < one_year * 3){
        nSubsidy=70 * COIN;
    }else if(nPrevHeight < one_year * 4){
        nSubsidy=60 * COIN;
    }else if(nPrevHeight < one_year * 5){
        nSubsidy=50 * COIN;
    }else if(nPrevHeight < one_year * 6){
        nSubsidy=40 * COIN;
    }else if(nPrevHeight < one_year * 7){
        nSubsidy=30 * COIN;
    }else if(nPrevHeight < one_year * 8){
        nSubsidy=20 * COIN;
    }else if(nPrevHeight < one_year * 9){
        nSubsidy=10 * COIN;
    }else if(nPrevHeight < one_year * 10){
        nSubsidy=9 * COIN;
    }else if(nPrevHeight < one_year * 11){
        nSubsidy=8 * COIN;
    }else if(nPrevHeight < one_year * 12){
        nSubsidy=7 * COIN;
    }else if(nPrevHeight < one_year * 13){
        nSubsidy=6 * COIN;
    }else if(nPrevHeight < one_year * 14){
        nSubsidy=5 * COIN;
    }else if(nPrevHeight < one_year * 15){
        nSubsidy=4 * COIN;
    }else if(nPrevHeight < one_year * 31){
        nSubsidy=3 * COIN;
    }else{
        nSubsidy=2 * COIN;
    }
...
{
    CAmount ret = blockValue * 0.3; // start at 20%
    return ret;
}
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // CG : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if(pindex->pprev->nHeight == 1500){
        blockReward = blockReward+50000000*COIN;
    }
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(CG): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(phashdb->GetHash(block), GetTime()));
        return state.DoS(0, error("ConnectBlock(CG): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CG : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(phashdb->GetHash(block), GetTime()));
                    return state.DoS(0, error("CheckBlock(CG): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CG): spork is off, skipping transaction locking checks\n");
    }
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckpointsEnabled && !CheckIndexAgainstCheckpoint(pindexPrev, state, chainparams, phashdb->GetHash(block)))
        return error("%s: CheckIndexAgainstCheckpoint(): %s", __func__, state.GetRejectReason().c_str());
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        {
                            LogPrint("reindex", "%s: Processing out of order child %s of %s\n", __func__, phashdb->GetHash(block).ToString(),
                                    head.ToString());
                            LOCK(cs_main);
                            ...
                            {
                                nLoaded++;
                                queue.push_back(phashdb->GetHash(block));
                            }
                        }
                        range.first++;
